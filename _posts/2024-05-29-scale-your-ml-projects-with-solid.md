...
두 번째로 SOLID 원칙을 적용하는 사람은 Python과 같은 상속 및 객체 지향 프로그래밍을 사용하여 깊은 계충 구조를 만들 때 쉽게 길을 잃을 수 있습니다. 실용주의 프로그래머 책에서는 베이스 클래스와 서브클래스를 넘어서 확장하지 말라고 조연하며 이런 접근 방식을 권장하지 않습니다. 네 번째 SOLID 원칙인 인터페이스 분리 원칙은 이러한 관행을 암묵적으로 권장하지 않습니다.